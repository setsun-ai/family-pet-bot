...
from petbot.family import FamilyService, Member
...
from petbot.sports import SportsService, Team

TOKEN = "987654321:" + "A" * 35
ME = User(id=987654321, is_bot=True, first_name="Whiskers", username="whiskers_test_bot")
OWNER, FAMILY = 100, -1000
TEAM = Team(key="riverside", name="Riverside", command="riverside", source="thesportsdb", team_id="1001", hour=9)
MEMBERS = (Member("Anna", "loves drawing", "03-14"), Member("Mom", "feeds the cat"), Member("Dad", "cooks on Sundays"))
...
class Harness:
    async def open(self, teams=(TEAM,), members=MEMBERS, **overrides) -> Harness:
        set_language("en")
        ...
        self.ai.post.return_value = "Go team!\n\nMeow"
        self.ai.check.return_value = "OK"
        self.ai.last_error = None
        self.ai.last_success = None
        self.delivery = DeliveryService(self.bot, self.db, self.settings.persona_emoji, pauses=False)
        self.news = NewsService(self.settings, self.db, self.ai, self.client, self.delivery)
        self.sports = SportsService(self.settings, self.client, self.ai, tuple(teams))
        self.family = FamilyService(self.settings, self.db, self.ai, tuple(members))
        self.access = AccessControl(self.db)
        self.scheduler = Scheduler(self.settings, self.db, self.news, self.sports, self.delivery, self.family)
        self.app = App(self.settings, self.db, self.bot, ME, self.ai, self.news, self.sports,
                       self.delivery, self.access, self.scheduler, RateLimiter(), ChatLocks(), family=self.family)
        return self
